...
public class SeamCarver {
	...
	public int[] findHorizontalSeam() {
		...
		int[] horizontalSeam = new int[savedEnergy[0].length - 1];
		
		double lowestEnergy = energyHorizontal[0][energyHorizontal[0].length - 1];
		for(int i = 0; i < energyHorizontal.length - 1; i++) {
			if(energyHorizontal[i][energyHorizontal[0].length - 1] < lowestEnergy) {
				lowestEnergy = energyVertical[i][energyHorizontal[0].length - 1];
				horizontalSeam[horizontalSeam.length - 1] = i;				
			}
		}
		
		for(int i = 1; i <  horizontalSeam.length; i++) {
			int yi1, yi2, yi3;
			double yv1, yv2, yv3;
			lowestEnergy = Double.MAX_VALUE;
			int lowestIndex;
			
			yi1 = horizontalSeam[horizontalSeam.length - i] - 1;
			yi2 = horizontalSeam[horizontalSeam.length - i];
			yi3 = horizontalSeam[horizontalSeam.length - i] + 1;
			
			yv2 = energyHorizontal[yi2][horizontalSeam.length - 1 - i];
			
			if(yi1 < 0) 
				yv1 = Double.MAX_VALUE;
			else
				yv1 = energyHorizontal[yi1][horizontalSeam.length - 1 - i];

			if(yi3 > savedEnergy.length - 1) 
				yv3 = Double.MAX_VALUE;
			else
				yv3 = energyHorizontal[yi3][horizontalSeam.length - 1 - i];
			...
			horizontalSeam[horizontalSeam.length - 1 - i] = lowestIndex;
	...
	public void removeHorizontalSeam(int[] indices) {
		if(indices.length < savedEnergy[0].length - 1) throw new IllegalArgumentException("Indices must be of length " + (savedEnergy[0].length - 1));
		if(this.height() == 1) throw new IllegalArgumentException("Cannot reduce image size");
	...
	public void removeVerticalSeam(int[] indices) {
		if(indices.length < savedEnergy.length - 1) throw new IllegalArgumentException("Indices must be of length " + height() + " not: " + indices.length);
		if(width() == 1) throw new IllegalArgumentException("Cannot reduce image size");

		
		for(int i = 0; i < indices.length; i++) {
			//System.out.println(indices[i]);
			if(indices[i] + 1 < this.width()) {
				pic.set(indices[i], i, pic.get(indices[i] + 1, i));

				for(int j = indices[i] + 1; j < this.width() - 1; j++) {
					pic.set(j, i , pic.get(j + 1, i));
					if(j == this.width() - 2)
					{
						pic.set(j + 1, i, new Color(0,0,0));
					}
				}
			}
		}
		...
		Picture smallerPic = new Picture(this.width() - 1, this.height());
